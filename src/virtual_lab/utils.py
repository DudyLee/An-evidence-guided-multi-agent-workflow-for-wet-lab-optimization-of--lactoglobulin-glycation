...
def run_tools(run: Run, all_agents_tools: dict | None = None) -> list[dict[str, str]]:
    """Runs the tools in a required action.

    :param run: The run to run tools for.
    :param all_agents_tools: A dictionary mapping tool name to the actual tool object.
    :return: A list of tool outputs.
    """
    if all_agents_tools is None:
        all_agents_tools = {}
    ...
    for tool in run.required_action.submit_tool_outputs.tool_calls:
        ...
        if tool_name == PUBMED_TOOL_NAME:
        ...
        elif tool_name in all_agents_tools:
            # LlamaIndex tool
            tool_to_run = all_agents_tools[tool_name]
            # LlamaIndex QueryEngineTool usually expects 'input'
            # But let's check what arguments are passed. Usually it's 'input' or 'query'.
            # The to_openai_tool() method defines the schema.
            # For QueryEngineTool, it's usually 'input'.
            query_input = args_dict.get("input", args_dict.get("query", ""))
            
            try:
                response = tool_to_run(query_input)
                output = str(response)
            except Exception as e:
                output = f"Error running tool {tool_name}: {e}"
            ...
            # Instead of raising error, maybe return error message to LLM?
            # But original code raises ValueError. Let's keep it consistent or just log error.
